...
The app opens on the Ask screen and is fully functional immediately — no
...
To enable live Yelp data:

1. Copy `.env.example` to `.env` or `Config/PointPilot.env` in `~/Developer/PointPilot/`:
   ```bash
   cp Config/.env.example .env
   ```
2. Open `.env` and add your Yelp Fusion API key:
   ```bash
   YELP_API_KEY=your_real_yelp_fusion_api_key_here
   ```
3. Run the app. PointPilot will automatically load your `.env` key securely at runtime via `AppEnvironment.swift`. If no `.env` or key is present, it seamlessly falls back to offline sample dining venues.
...
**No API key or agent secret is ever committed.** Configuration is read at
runtime from a gitignored plist. If it is missing or still holds placeholders,
the app runs in demo mode.

1. Add the ElevenLabs Swift SDK in Xcode:
   **File ▸ Add Package Dependencies…** →
   `https://github.com/elevenlabs/elevenlabs-swift-sdk.git` (2.0.0 or later)
2. Copy the example config to the gitignored path and fill in your agent ID:

   ```bash
   cp Config/ElevenLabs.example.plist PointPilot/Config/ElevenLabs.plist
   ```

3. Configure your agent in the ElevenLabs dashboard with a **client** tool named
   `recommendCard` accepting `merchant` (string) and `amount` (number).
4. Build. `ElevenLabsVoiceAgentService` is compiled only when the SDK is present
   (`#if canImport(ElevenLabs)`), so the project builds with or without it.

`Config/ElevenLabs.example.plist` contains placeholders only and is safe to
commit. Both `PointPilot/Config/ElevenLabs.plist` and `Config/ElevenLabs.plist`
are gitignored.
...
└── Views/           AskView, RecommendationView, WalletView,
                     AskViewModel, Theme
```
...
**53 tests, all passing**: 42 unit tests (engine, voice transcript, Yelp Fusion DTOs, .env parser, Location, discovery) and 11 UI tests for the end-to-end demo flow.
...
- `YelpFusionServiceTests.swift`: 3 tests for Yelp API DTO mapping, category inference, and offline fallback.
- `LocationAndDiscoveryTests.swift`: 3 tests for on-demand GPS, one-tap chip selection, and the no-location-fix fallback.

### UI tests (`PointPilotUITests/DemoFlowUITests.swift`)
- 11 automated UI tests driving the real app: typed input, nearby merchant chips, breakdown expansion, capped offer adjustments, simulated activation, wallet sheet, error banners, screen resets, repeated microphone taps, and transcript timestamps.

```
Executed 42 tests, with 0 failures     (unit)
Executed 11 tests, with 0 failures     (UI)
...
- **Sample data only.** Three cards, five merchants. No real issuer data.
...
- **Dining is the only category exercised.** The model supports groceries,
  travel, gas and other, but sample offers exist only for dining.
...
1. **Launch PointPilot.** The Ask screen detects nearby restaurants with GPS and shows one-tap quick-selection chips (e.g. Nobu, Shake Shack) alongside the large voice button.
2. **Tap a nearby chip (or tap the voice button).**
   - If tapping **Nobu**: auto-populates the restaurant name. Type `200` in amount.
   - If speaking: say *"I'm at Nobu and spending around $200."*
4. **The Amex Gold card appears** with a success haptic and **$45.00** estimated value.
...
9. **Tap "Ask Another."** Back to a clean Ask screen.
...
- Value is never communicated by colour alone — deductions carry an explicit `−`
  sign and a label; the winner is named, not just highlighted.
